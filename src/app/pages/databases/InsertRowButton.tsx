...
function isAutoGeneratedPrimaryKey(column: TableColumnInfo) {
  return column.isPrimaryKey && (column.defaultValue?.includes("nextval(") ?? false);
}
...
function isRequiredColumn(column: TableColumnInfo) {
  if (isAutoGeneratedPrimaryKey(column)) return false;
...
export function InsertRowButton({ connectionId, schemaName, tableName, columns, canMutate }: Props) {
  ...
  const editableColumns = columns.filter((column) => !isAutoGeneratedPrimaryKey(column));
  ...
  const onInsert = (event: React.FormEvent<HTMLFormElement>) => {
    ...
    for (const column of editableColumns) {
      ...
      if (isJsonInputColumn(column)) {
        try {
          values[column.name] = JSON.parse(trimmed) as unknown;
        } catch {
